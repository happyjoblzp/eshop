...
class sellerDecorate extends pluginBase
{
	...
	public $actions = ['site' => ['home','sellerCate']];
        ...
        plugin::reg("onBeforeCreateAction@seller@ad_edit_act",function(){
            self::controller()->ad_edit_act = function(){
                ...
                else
                {
                    ...
                    {
                        IError::show(403,"图片广告和flash广告必须上传附件！");
                    }
                }
	...
	public function home()
	{
		...
		$this->theme = $this->getTheme($seller_id);
		if($this->theme)
		{
	...
	public function sellerCate()
	{
        ...
        $catRow = $catObj->getObj('id = '.$cat_id);
        if(!$catRow)
        {
            IError::show(403,'商户分类信息不存在');
        }

        $this->theme = $this->getTheme($catRow['seller_id']);
        if($this->theme)
        {
            $sellerRow = Api::run('getSellerInfo',$catRow['seller_id']);
            if(!$sellerRow)
            {
                IError::show(403,'商户信息不存在');
            }
            ...
            $this->controller()->layout = $this->getLayout();
            $this->controller()->setRenderData(array('sellerRow' => $sellerRow,'catRow' => $catRow,'seller_id' => $catRow['seller_id']));
            $this->redirect('sellerCate');
        }
	}
	...
	public function getSellerTheme()
	{
		...
		if(is_dir($planPath))
		{
			...
			while(false !== ($dir = readdir($dirRes)))
			{
                ...
                $tempData     = file_exists($fileName) ? include($fileName) : array('ad'=>array());
				$result[$dir] = $tempData;
			}
		}
		return $result;
